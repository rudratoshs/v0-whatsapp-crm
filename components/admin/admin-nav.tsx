    ...
    <aside className="w-64 bg-sidebar border-r border-sidebar-border p-6 flex flex-col">
      <div className="flex items-center gap-2 mb-8">
        ...
        <div>
          ...
          <p className="text-xs text-sidebar-accent-foreground">Admin</p>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-2">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="block px-4 py-2 rounded-lg text-sidebar-foreground hover:bg-sidebar-accent transition-colors"
          >
            {item.label}
          </Link>
        ))}
